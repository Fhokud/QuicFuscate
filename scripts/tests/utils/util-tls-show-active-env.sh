...
F=${QUICFUSCATE_USE_TLS_COVER:-${QUICFUSCATE_USE_TLS_COVER:-0}}
D=${QUICFUSCATE_DOH:-1}
...
echo -n '  {"browser":'"\"$B\""',"os":'"\"$O\""',"use_tls_cover":'"\"$F\""',"doh_enabled":'"\"$D\""',"doh_provider":'"\"$FP\""',"fronting":'"\"$FR\""',"qpack":'"\"$Q\""',"xor_obfuscation":'"\"$X\""'}' >> "$JSON"
