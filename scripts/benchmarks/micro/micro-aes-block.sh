#!/usr/bin/env bash
# Description: Micro-benchmark runner: micro-aes-block.
set -euo pipefail

# Microbench: AES-128 block encryption throughput
...
print_system_banner
info "AES-Block microbench sizes: ${SIZES[*]} | iters=$ITERS"
...
for sz in "${SIZES[@]}"; do
  run cargo run --release -q --example microbench -- aes-block "$sz" "$ITERS" | tee -a "$OUT_CSV"
  echo "---" | tee -a "$OUT_CSV"
done
