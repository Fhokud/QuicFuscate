#!/usr/bin/env bash
# Description: Micro-benchmark runner: micro-aes-gcm.
set -euo pipefail

# Microbench: AES-GCM seal (encrypt + tag)
...
for sz in "${SIZES[@]}"; do
  run cargo run --release -q --example microbench -- aes-gcm "$sz" "$ITERS" | tee -a "$OUT_CSV"
  echo "---" | tee -a "$OUT_CSV"
done
