#!/usr/bin/env bash
# Description: Micro-benchmark runner: micro-chacha-x4.
set -euo pipefail

# Microbench: ChaCha20 4-way keystream blocks (SIMD)
# Uses examples/microbench.rs as driver. Policy: not used for payload AEAD.
...
for sz in "${SIZES[@]}"; do
  run cargo run --release -q --example microbench -- chacha-x4 "$sz" "$ITERS" | tee -a "$OUT_CSV"
  echo "---" | tee -a "$OUT_CSV"
done
