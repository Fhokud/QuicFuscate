...
function persistCsrfToken(token: string | null): void {
  ...
  try {
    ...
    // Ignore storage errors (private mode / policy); in-memory token still works.
  }
}
...
async function ensureCsrfToken(forceRefresh = false): Promise<void> {
  ...
  try {
    ...
    // Network issues are handled by the actual request path.
  }
}
...
export function parseErrorMessageBody(text: string): string | null {
  ...
  // Try to parse common JSON error shapes.
  if (trimmed.startsWith("{") || trimmed.startsWith("[") || trimmed.startsWith("\"")) {
    try {
      ...
      // fall through to returning truncated text
    }
  }

  // Fallback to plain text body (truncated).
...
async function request(path: string, init: RequestInit): Promise<Response> {
  ...
  if (!resp.ok) {
    ...
    if (resp.status === 423) {
      // Let the app lock the UI to Settings (password change required).
      if (typeof window !== "undefined") {
        window.dispatchEvent(new CustomEvent("qf:admin-password-change-required"));
      }
    }
    throw new ApiError(
