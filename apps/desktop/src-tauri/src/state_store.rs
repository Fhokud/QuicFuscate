...
use crate::{
    hydrate_state_for_runtime, migrate_legacy_state, redact_state_for_disk, LegacyPersistedState,
    PersistedState,
};
use rand::RngCore;
use std::fs::File;
    ...
    pub(crate) fn load_state_from_path(
        ...
        let process = |raw: &str| -> Result<(PersistedState, String), String> {
            ...
            let legacy: LegacyPersistedState =
                serde_json::from_str(raw).map_err(|e| format!("State parse failed: {}", e))?;
            let migrated = migrate_legacy_state(legacy);
            let runtime = hydrate_state_for_runtime(migrated, store);
            let disk = redact_state_for_disk(runtime.clone(), store);
            let json = serde_json::to_string_pretty(&disk).map_err(|e| e.to_string())?;
            Ok((runtime, json))
        };
