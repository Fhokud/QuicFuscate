...
// Migration: legacy schema from early desktop stub (WireGuard-like tunnels).
#[derive(Clone, Debug, Default, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct LegacyPersistedState {
...
pub(crate) fn migrate_legacy_state(legacy: LegacyPersistedState) -> PersistedState {
    let mut out = PersistedState {
        schema_version: 1,
        tunnels: Vec::new(),
        selected_tunnel_id: legacy.selected_tunnel_id,
        settings: legacy.settings,
    };

    for t in legacy.tunnels {
        let id = t.get("id").and_then(|v| v.as_str()).unwrap_or("").to_string();
        let name = t.get("name").and_then(|v| v.as_str()).unwrap_or("Imported").to_string();
        let qk = t.get("qkey").and_then(|v| v.as_str()).unwrap_or("").to_string();
        let country_code = t.get("countryCode").and_then(|v| v.as_str()).map(|s| s.to_string());
        let location = t.get("location").and_then(|v| v.as_str()).map(|s| s.to_string());

        let endpoint = t.get("endpoint").and_then(|v| v.as_str()).unwrap_or("").to_string();
        let host = endpoint.split(':').next().unwrap_or("").to_string();

        // Migration policy: preserve the tunnel shell (id/name/endpoint) even if the legacy qkey is
        // missing or malformed, but do not carry forward invalid qkeys. This keeps the UI usable and
        // forces the user to paste a correct QKey before connecting.
        let (remote, sni, has_token, qkey_value) = if !qk.trim().is_empty() {
            match qkey::parse(&qk) {
                Ok(parsed) => {
                    let has_token =
                        parsed.token.as_deref().map(is_valid_token_hex_32).unwrap_or(false);
                    (
                        parsed.remote,
                        parsed.sni,
                        has_token,
                        if has_token { qk } else { String::new() },
                    )
                }
                Err(_) => (endpoint.clone(), host.clone(), false, String::new()),
            }
        } else {
            (endpoint.clone(), host.clone(), false, String::new())
        };

        if id.trim().is_empty() || remote.trim().is_empty() || sni.trim().is_empty() {
            continue;
        }

        out.tunnels.push(PersistedTunnel {
            id,
            name,
            remote,
            sni,
...
fn build_client_engine_config(
    ...
    if let Some(ref stealth) = qk.stealth {
        ...
        cfg.stealth.mode = match mode.as_str() {
            ...
            "max" => quicfuscate::engine::StealthMode::Max,
            "anti-dpi" | "antidpi" => quicfuscate::engine::StealthMode::Max,
    ...
    if let Some(ref fec) = qk.fec {
        ...
        cfg.fec.mode = match mode.as_str() {
            "off" | "zero" => quicfuscate::engine::FecMode::Off,
            "manual" | "on" => quicfuscate::engine::FecMode::Manual,
...
mod tests {
    ...
    #[test]
    fn config_builder_uses_qkey_manual_fec_even_if_desktop_requests_off() {
        let qk = quicfuscate::engine::qkey::generate(
            &quicfuscate::engine::qkey::QKeyConfig::new("not-an-addr", "example.com")
                .with_token(&"a".repeat(64))
                .with_stealth("auto")
        ...
        let cfg = build_client_engine_config(&qk, None, Some(&settings)).expect("cfg");
        assert_eq!(cfg.fec.mode, quicfuscate::engine::FecMode::Manual);
        assert_eq!(cfg.logging.level, "debug");
    }

    #[test]
    fn config_builder_accepts_qkey_fec_on_alias() {
        let qk = quicfuscate::engine::qkey::generate(
            &quicfuscate::engine::qkey::QKeyConfig::new("not-an-addr", "example.com")
                .with_token(&"a".repeat(64))
                .with_fec("on"),
    ...
    fn config_builder_accepts_qkey_stealth_antidpi_alias() {
        ...
        let settings = mk_settings_with_connection("auto", "auto");
        let cfg = build_client_engine_config(&qk, None, Some(&settings)).expect("cfg");
        assert_eq!(cfg.stealth.mode, quicfuscate::engine::StealthMode::Max);
    }
    ...
    #[test]
    fn legacy_state_migration_keeps_valid_tunnels_only() {
        // This matches the legacy schema shape used by the loader.
        let good_qkey = quicfuscate::engine::qkey::generate(
            &quicfuscate::engine::qkey::QKeyConfig::new("1.2.3.4:4433", "example.com")
                .with_token(&"a".repeat(64)),
        );
        let legacy = serde_json::json!({
            "tunnels": [
                { "id": "t1", "name": "A", "qkey": "QKey-invalid", "endpoint": "1.2.3.4:4433" },
                { "id": "t2", "name": "B", "qkey": "", "endpoint": "1.2.3.4:4433" },
                { "id": "t3", "name": "C", "qkey": good_qkey, "countryCode": "DE", "location": "Berlin" }
            ],
            "settings": { "general": { "logLevel": "info" } },
            "selectedTunnelId": "t3"
        });
        let data = legacy.to_string();

        let legacy: LegacyPersistedState = serde_json::from_str(&data).unwrap();
        let out = migrate_legacy_state(legacy);

        // t1 and t2 are preserved as shells but have no usable qkey after migration.
        // t3 keeps the valid qkey.
        assert_eq!(out.tunnels.len(), 3);
        let t1 = out.tunnels.iter().find(|t| t.id == "t1").unwrap();
        let t2 = out.tunnels.iter().find(|t| t.id == "t2").unwrap();
        let t3 = out.tunnels.iter().find(|t| t.id == "t3").unwrap();
        assert!(t1.qkey.is_empty());
        assert!(t2.qkey.is_empty());
        assert!(t3.qkey.starts_with("QKey-"));
        assert_eq!(out.selected_tunnel_id.as_deref(), Some("t3"));
    }
