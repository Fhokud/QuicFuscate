...
function getStageAnchorFallback(): ToastAnchor {
  if (typeof document === "undefined") {
    return { x: 0, y: 42 };
  }
  const stage = document.getElementById("qf-app-stage");
  if (!stage) {
    return { x: Math.round(window.innerWidth / 2), y: 42 };
  }
  const rect = stage.getBoundingClientRect();
  return { x: Math.round(rect.left + rect.width / 2), y: Math.round(rect.top + 42) };
}
...
export function ToastContainer() {
  ...
  const effectiveAnchor = lockedToast?.anchor ?? anchor ?? getStageAnchorFallback();
