...
export function parseRemote(remote: string): { server: string; port: number } | null {
  ...
  // Reject obvious invalid input early.
  ...
  // Unbracketed IPv6 is ambiguous with ports. Force bracketed form.
...
/**
 * SNI must be a host only - no spaces, no URL separators, no port.
 */
